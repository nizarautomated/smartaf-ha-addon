...
- Clear processed deployment, proposal, entity-diagnostic, log-diagnostic and command request pointers only after their terminal report has been published.
- Retry safe pointer cleanup when a previously processed request is still present.
- Preserve immutable status and report files, the current flow baseline, local state and backups.
- Add no configuration fields and keep all existing stored options valid.
